    ...
    [
        [
            InlineKeyboardButton(   
              text=f"برود •", url=f"https://t.me/zzzzx1",)                        
        ],        
    ]
)
async def check_is_joined(message):    
    try:
        ...
        status = await app.get_chat_member("zzzzx1", userid)
        return True
    except Exception:
        await message.reply_text("⚠️︰ عذرا، عليك الانضمام الى قناة البوت أولاً :",reply_markup=force_btn,parse_mode="markdown",disable_web_page_preview=False)
        return False
